...
A fundamental concept in the theory of dynamical systems is the Lyapunov exponent. Loosely speaking, Lyapunov exponents (LEs) quantify how nearby trajectories in the state space of a nonlinear dynamical system diverge (or converge) over time. In general, LEs must be computed numerically along specific trajectories of the system under consideration. If the system is stable -- for instance, if it is contractive -- it may be possible to conclude _a priori_ that the LEs are negative. However, assigning precise numerical values to the LEs cannot in general be done analytically. Instead, LEs must be estimated by repeatedly multiplying Jacobian matrices of the dynamics. However, this approach can be numerically unstable if the underlying dynamics themselves are unstable. Methods that address this instability, such as iterative QR decomposition, are not parallelizable in time.
